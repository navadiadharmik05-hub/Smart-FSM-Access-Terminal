...
> **Total BOM:** Rs.788 | **Target Sequence:** 1-0-1-1 (reconfigurable) | **License:** MIT
...
10. [Safety Interlock Subsystems](#10-safety-interlock-subsystems)
...
This project presents a pedagogical yet **industrial-grade Hybrid Synchronous Finite State Machine**
**(FSM)** sequence detector built from discrete 7400-series CMOS ICs and supervised by an ESP8266
NodeMCU over a live Wi-Fi REST API. The design directly bridges foundational D flip-flop and gate
theory with modern IoT SCADA supervisory architectures.
...
| **FSM Type** | Hybrid Moore/Mealy synchronous, positive-edge-triggered |
| **Safety Layers** | Progressive lockout, tamper detection, OTP, duress protocol |
...
|:---:|:---|:---:|:---:|:---:|
| **S0** | System Armed / Idle Standby | `0 0 0 0` | Stay S0 | Advance to S1 |
| **S1** | 1st bit matched (`1`) | `0 0 0 1` | Advance to S2 | Stay S1 |
| **S2** | 2nd bit matched (`1-0`) | `0 0 1 0` | Return to S0 | Advance to S3 |
| **S3** | 3rd bit matched (`1-0-1`) | `0 1 0 0` | Return to S0 | Advance to S4 |
...
|-- mock_esp8266_server.py           Python mock hardware server (stdlib only, no pip needed)
...
**Step 1: Clone the repository**
    ...
    git clone https://github.com/YOUR_USERNAME/fsm-smart-access-terminal.git
    cd fsm-smart-access-terminal
...
**Step 1: Start the mock hardware server (Python stdlib -- zero dependencies)**
...
**Step 2: Open the Supervisor Dashboard**
    ...
    1. Open fsm_supervisor_dashboard.html in any modern browser
    2. Set server URL to: http://127.0.0.1:8080
    3. Use the dashboard to inject codes, monitor flip-flop states, trigger OTP

**Windows One-Click Launch:**

    launch_smartlock.bat
    (Starts mock server + opens dashboard automatically)
...
### Sample Status Response
    ...
    { status: ONLINE, target: 1011, currentState: S4,
      flipFlops: [1,0,0,0], hardwareQ3Sense: 1,
      strikeCount: 0, isLockedOut: false, uptimeSeconds: 347 }

---

## 8. Automated Test Suite

The Node.js test suite (`test_fsm_engine.js`) runs an exhaustive **16-combination input matrix**
across 4 different reconfigurable target sequences -- total of 64 test cases.
...
The deterministic FSM simulation verifies the one-hot Boolean equations produce exactly one
UNLOCK event per 16-combination sweep (6.25% unlock rate) with zero erroneous transitions.
...
| Discrete Passives | 330-ohm + 10k-ohm resistors & decoupling capacitors | 8 | 1.00 | 8.00 |
| Breadboard + Jumpers | 830-point solderless + assorted wire kit | 1 | 160.00 | 160.00 |
| **TOTAL** | | | | **Rs. 788.00 (~USD 9.50)** |

---

## 10. Safety Interlock Subsystems
...
| 3rd wrong code | LOCKOUT ENGAGED | **LOW (GND)** | 30 seconds |
| 4th+ (post reset) | Escalating lockout | **LOW (GND)** | 60s -> 120s (doubles each time) |

When locked out, the /CLR bus is held permanently at GND, instantly discharging all internal
master-slave latches and ignoring all external clock pulses until the timer expires.

### Layer 2 -- Chassis Tamper Isolation (GPIO13)

- A physical microswitch on the enclosure is wire to GPIO13 (active LOW).
- Physical tamper triggers a non-maskable interrupt: /CLR is pulled LOW immediately.
- Acoustic buzzer activates; all REST responses return `isTampered: true`.
- Prevents logic analyzer probing by physically isolating the flip-flop state registers.

### Layer 3 -- Duress Silent Panic Protocol

- Reserved duress code unlocks the gate mechanism visually (no alert visible to intruder).
- Simultaneously dispatches a silent encrypted incident packet to the supervisor console.
- The attacker observes normal unlock behavior while the alert is silently escalated.
...
- Enables temporary field-technician access without revealing the master passcode.
- OTP is automatically invalidated after: (a) one successful use, or (b) 60-second expiry.
...
| **Active-LOW /CLR** | Industry-standard fail-safe default. A power-interruption resets the FSM to S0 (Idle) without a dedicated reset pulse generator. |
| **74HC-series over 74LS** | HC CMOS is 3.3V/5V compatible, has lower static power consumption, and directly interfaces with ESP8266 GPIO levels without level-shifting. |
...
| **I2C OLED over SPI** | Saves 3 GPIO pins compared to SPI. Critical on NodeMCU which has limited available GPIOs for simultaneous FSM driver + display + relay. |
| **Wi-Fi Soft-AP** | No external router dependency. The device creates its own access point for standalone field deployment without infrastructure. |
| **EEPROM Reconfiguration** | Target pattern is reprogrammable at runtime via authenticated REST API without reflashing firmware -- extends hardware life. |
...
|:---:|:---|
| **SDG 9** -- Industry, Innovation and Infrastructure | Demonstrates a modular, reconfigurable hardware FSM topology that can be re-targeted for industrial assembly lines, access control systems, and digital frame synchronization applications without discarding or replacing any silicon. |
| **SDG 12** -- Responsible Consumption and Production | Minimizes electronic waste by enabling dynamic runtime EEPROM reconfiguration, multi-target testbench reuse across 4 different code patterns, and a complete design built from standard off-the-shelf TTL/CMOS 7400-series ICs that remain in production for decades. |
...
| `FSM_SIM_dashboard.mp4` | Browser-based supervisor dashboard walkthrough and code injection demo |
| `VLAB_implementation_TinkerCad.mp4` | Virtual lab TinkerCad circuit simulation of the 74HC74 FSM |

> All demo videos are stored in the repository root. Clone the repo and play locally.
> For GitHub bandwidth: consider uploading to YouTube and linking here.

---

## 14. How It Works -- Step by Step

1. User types a 4-bit binary code in the supervisor dashboard and clicks Inject.
2. ESP8266 pulls **/CLR HIGH** (re-arms the flip-flop chain to accept new state).
3. For each bit, ESP8266 sets the **DATA line** (D6/GPIO12) HIGH or LOW, then sends a **clock pulse** (D5/GPIO14 rising edge).
4. On each rising edge, the **74HC08 AND gate array** computes D0-D3 from current Q values.
5. The **74HC04 inverter** supplies the required complement signal ~X.
6. The **74HC74 flip-flops** latch the new state, stepping S0 -> S1 -> S2 -> S3 -> S4.
7. At **S4**: Q3 goes HIGH, lighting the Green LED, actuating the relay, and GPIO16 reads HIGH.
8. OLED shows **UNLOCKED / ACCESS GRANTED**; REST API returns `hardwareResult: UNLOCKED`.
9. After 2.5 seconds, ESP8266 pulses **/CLR LOW** to reset all flip-flops back to S0.
...
| Role | Contributor |
|:---|:---|
| Hardware Design and IC Wiring | |
| FSM Logic and Boolean Derivation | |
| ESP8266 Firmware (Arduino C++) | |
| Supervisor Dashboard (HTML/JS) | |
| Mock Server and Test Suite | |
| Documentation and SDG Analysis | |

> Fill in team member names before publishing to GitHub.
